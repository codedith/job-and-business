...
public class MesiboSampleAPI {
    private static final String TAG="SampleAPI";
    private static MesiboNotifyUser mNotifyUser = null;
    ...
    public static abstract class ResponseHandler implements Mesibo.HttpListener {
        ...
        public boolean Mesibo_onHttpProgress(Mesibo.Http http, int state, int percent) {
            ...
            if(100 == percent && Mesibo.Http.STATE_DOWNLOAD == state) {
                ...
                final Context context = (null == this.mContext)?MesiboSampleAPI.mContext:this.mContext;
...
//        return  ContactUtils.reverseLookup(phone);
    ...
    public static void createContact(String name, String phone,  long groupid, String status, String members, String photo, String tnBasee64, long ts, long when, boolean selfProfile, boolean refresh, int visibility) {
        ...
        if(selfProfile) {
            MesiboAppConfig.getConfig().phone = u.address;
    ...
    private static boolean parseResponse(Response response, Bundle request, Context context, boolean uiThread) {
        ...
        if(null != response.urls) {
            MesiboAppConfig.getConfig().uploadurl = response.urls.upload;
            MesiboAppConfig.getConfig().downloadurl = response.urls.download;
            save = true;
        }

        if(null != response.share) {
            MesiboAppConfig.getConfig().invite = response.share;
            save = true;
        }

        if(response.op.equals("login") && !TextUtils.isEmpty(response.token)) {
            MesiboAppConfig.getConfig().token = response.token; //TBD, save into preference
            MesiboAppConfig.getConfig().phone = response.phone;
            MesiboAppConfig.getConfig().cc = response.cc;
            mContactTs = 0;
            ...
            forceLogout();
            MesiboAppConfig.reset();
        }

        if(save)
            MesiboAppConfig.save();
        return true;
    }
...
//        UIManager.showAlert(context, title, message);
    ...
    public static void saveLocalSyncedContacts(String contacts, long timestamp) {
        Mesibo.setKey(MesiboSampleAPI.KEY_SYNCEDCONTACTS, contacts);
        Mesibo.setKey(MesiboSampleAPI.KEY_SYNCEDDEVICECONTACTSTIME, String.valueOf(timestamp));
    }

    public static void saveSyncedTimestamp(long timestamp) {
        Mesibo.setKey(MesiboSampleAPI.KEY_SYNCEDCONTACTSTIME, String.valueOf(timestamp));
    }
    ...
    public static void init(Context context) {
        ...
        if(!TextUtils.isEmpty(MesiboAppConfig.getConfig().token)) {
            startMesibo(false, context);
            startSync();
        }
    }

    public static String getPhone() {
        if(!TextUtils.isEmpty(MesiboAppConfig.getConfig().phone)) {
            return MesiboAppConfig.getConfig().phone;
        }
    ...
    public static String getToken() {
        return MesiboAppConfig.getConfig().token;
    }
    public static String getUploadUrl() { return MesiboAppConfig.getConfig().uploadurl; }
    public static String getDownloadUrl() { return MesiboAppConfig.getConfig().downloadurl; }
    ...
    private static void startSync() {
        ...
        synchronized (MesiboSampleAPI.class) {
            if(!mSyncPending)
                return;
            mSyncPending = false;
        }

        MesiboSampleAPI.getContacts(null, false, true);
    }
...
//        ContactUtils.sync(synced, ts, false, MesiboListeners.getInstance());
    ...
    public static boolean startMesibo(boolean resetContacts, Context context) {
        ...
        mNotifyUser = new MesiboNotifyUser(context);

        // set access token
        if(0 != Mesibo.setAccessToken(MesiboAppConfig.getConfig().token)) {
            return false;
        }
        ...
        if(resetContacts) {
            MesiboSampleAPI.saveLocalSyncedContacts("", 0);
            MesiboSampleAPI.saveSyncedTimestamp(0);
        }
...
//        ContactUtils.setCountryCode(MesiboAppConfig.getConfig().cc);
        ...
        Mesibo.runInBackground(context, null, restartIntent);
    ...
    private static Bundle createPostBundle(String op) {
        ...
        Bundle b = new Bundle();
        b.putString("op", op);
        b.putString("token", MesiboAppConfig.getConfig().token);
        return b;
    }
    ...
    public static void forceLogout(){
        ...
        MesiboSampleAPI.saveSyncedTimestamp(0);
        Mesibo.stop(true);
        MesiboAppConfig.getConfig().reset();
...
//        UIManager.launchStartupActivity(mContext, true);
    ...
    public static void notify(Mesibo.MessageParams params, String message) {
        ...
        if(params.isMissedCall()) {
            ...
            MesiboSampleAPI.notify(MesiboNotifyUser.NOTIFYCALL_CHANNEL_ID, 2, subject, message);
            return;
        }
